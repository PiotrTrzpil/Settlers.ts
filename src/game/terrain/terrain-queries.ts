...
export function isBuildable(groundTypeValue: number): boolean {
    if (!isPassable(groundTypeValue)) {
        return false;
    }
    // Rock: 32 - only mines can build here (see isMineBuildable)
    if (groundTypeValue === 32) {
        return false;
    }
    // Beach: 48 - not buildable
    if (groundTypeValue === 48) {
        return false;
    }
    // Swamp: 80, 81 - not buildable
    if (groundTypeValue === 80 || groundTypeValue === 81) {
        return false;
    }
    ...
    if (groundTypeValue === 144 || groundTypeValue === 145) {
        return false;
    }
    // River: 96-99 - not buildable
    if (groundTypeValue >= 96 && groundTypeValue <= 99) {
        return false;
    }
