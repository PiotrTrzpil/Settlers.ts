...
function printResult(r: TreeResult): void {
    ...
    if (r.count > 0 && r.count < 500000) {
        ...
        for (const [type, count] of sorted) {
            ...
            console.log(
                `  ${name.padEnd(10)} (#${type.toString().padStart(2)}): ${count.toString().padStart(6)} (${pct}%)`
            );
        }
    }
}
...
async function main() {
    ...
    while (offset < buffer.length - 20) {
        // bytes 0-3 are the chunk offset (unused in this parser, content starts after header)
        const chunkType =
            buffer[offset + 4]! |
            (buffer[offset + 5]! << 8) |
            (buffer[offset + 6]! << 16) |
        ...
        if (chunkType === 1 && unpackedLen >= 16) {
            ...
            mapWidth =
                buffer[sizeOffset]! |
                (buffer[sizeOffset + 1]! << 8) |
                (buffer[sizeOffset + 2]! << 16) |
                (buffer[sizeOffset + 3]! << 24);
    ...
    if (bytesPerTile === 4) {
        // Byte 0
        results.push(analyzeApproach('INTERLEAVED: Byte 0', mapObjectsData, mapWidth, mapHeight, (d, i) => d[i * 4]!));

        // Byte 1
        results.push(
            analyzeApproach('INTERLEAVED: Byte 1', mapObjectsData, mapWidth, mapHeight, (d, i) => d[i * 4 + 1]!)
        );

        // Byte 2
        results.push(
            analyzeApproach('INTERLEAVED: Byte 2', mapObjectsData, mapWidth, mapHeight, (d, i) => d[i * 4 + 2]!)
        );

        // Byte 3
        results.push(
            analyzeApproach('INTERLEAVED: Byte 3', mapObjectsData, mapWidth, mapHeight, (d, i) => d[i * 4 + 3]!)
        );

        // Byte 0 where Byte 2 has flag (64 or 65)
        results.push(
            analyzeApproach('INTERLEAVED: Byte 0 where Byte2=64|65', mapObjectsData, mapWidth, mapHeight, (d, i) =>
        ...
        results.push(
            analyzeApproach('INTERLEAVED: Byte 0 where Byte2!=0', mapObjectsData, mapWidth, mapHeight, (d, i) =>
                d[i * 4 + 2] !== 0 ? d[i * 4]! : 0
            )
        );
        ...
        results.push(
            analyzeApproach('COMBINED: Byte0 OR Byte1', mapObjectsData, mapWidth, mapHeight, (d, i) => {
                const b0 = d[i * 4]!;
                const b1 = d[i * 4 + 1]!;
    ...
    const scored = results.map(r => {
        ...
        if (r.count >= idealMin && r.count <= idealMax) countScore = 100;
        else if (r.count < idealMin) countScore = (r.count / idealMin) * 50;
        else countScore = Math.max(0, 50 - ((r.count - idealMax) / idealMax) * 50);
