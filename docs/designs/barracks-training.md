...
Each non-DarkTribe race has a Barracks building that converts incoming weapons and gold into soldiers. Barracks receives materials (swords, bows, gold bars, armor, race-specific specialist weapons) via normal carrier delivery. When all inputs for a training recipe are present, the barracks consumes them, removes one idle carrier from the player's pool, and spawns the corresponding soldier unit near the barracks door. Recipe selection reuses the existing `ProductionControlManager` (even/proportional/manual modes), which is refactored from `EMaterialType`-keyed to recipe-index-keyed so it works for both material-producing buildings and unit-producing barracks.
...
3. If inputs are satisfied → consume inputs, start training timer
4. On timer completion → spawn soldier unit near door, remove one idle carrier from player
...
| 4 | Barracks Training Manager | Core training loop using PCM for recipe selection, training timer, soldier spawning, carrier consumption | `barracks/barracks-training-manager.ts`, `barracks/index.ts` |
| 5 | Integration & Wiring | Wire manager into game lifecycle, update spawn-on-complete and production entries | `game-services.ts`, `game.ts`, `spawn-units.ts`, `building-production.ts` |
| 6 | Tests | Verify PCM refactor, recipe configs, training logic, soldier spawning, carrier consumption | `barracks-training.spec.ts`, `production-system.spec.ts` updates |
...
| Field | Type | Description |
|-------|------|-------------|
| recipe | `TrainingRecipe` | The recipe currently being trained |
| progress | `number` | Training progress 0.0 → 1.0 |
...
```typescript
// Find and remove one idle carrier belonging to a player.
// Returns the entity ID of the consumed carrier, or null if none available.
// Prefers carriers nearest to the barracks.
function consumeIdleCarrier(
...
| Carrier consumption | No idle carrier available | Training completes but soldier spawn is **deferred** — state stays at progress=1.0 and retries next tick until a carrier is available. Materials are already consumed. |
| Soldier spawning | No valid tile near barracks door | Use spiral search (existing pattern from `spawn-units.ts`). If all tiles blocked within radius 4, defer to next tick. |
| Building destroyed mid-training | Building removed while training active | `removeBarracks()` cleans up state. In-progress training is lost (materials already consumed — this matches S4 behavior). |
...
**Owns**: Training lifecycle, timer progression, soldier spawning, carrier consumption.

**Depends on**: Subsystem 1 (PCM), Subsystem 3 (recipe types/configs), `BuildingInventoryManager`, `CarrierManager`, `GameState`.
...
- **Training duration**: Fixed at ~3 seconds per soldier (configurable constant `TRAINING_DURATION_SECONDS`). Same duration regardless of unit level.
- **Tick-driven, not event-driven**: Manager checks all barracks each tick rather than reacting to `inventory:changed` events. Simpler and avoids ordering issues.
- **One training at a time per barracks**: The next recipe is selected only after the current training completes.
...
1. If activeTraining exists:
   a. Advance progress by dt / TRAINING_DURATION_SECONDS
   b. If progress >= 1.0:
      - Try to consume one idle carrier (consumeIdleCarrier)
      - If no carrier available: keep state at progress=1.0, retry next tick
      - If carrier consumed: spawn soldier, clear activeTraining, emit event
2. If no activeTraining:
   ...
   e. If inputs satisfied:
      - Consume inputs: for each {material, count}, withdrawInput(buildingId, material, count)
      - Set activeTraining = { recipe, progress: 0 }
      - Increment PCM production count (already done by getNextRecipeIndex)
   f. If not satisfied: skip (will retry next tick)
```

Soldier spawning reuses the spiral pattern from `spawn-units.ts`:
- Get barracks door position (building entity position)
- Spiral search for a walkable, unoccupied tile
- Execute `SpawnUnitCommand` with correct `unitType`, `level`, `player`, `race`

Carrier consumption:
- Query `CarrierManager` for idle carriers belonging to the barracks's player
- Prefer carriers closest to the barracks (use `findNearestEntity` with Carrier filter + idle check)
- Remove the carrier entity from `GameState` and unregister from `CarrierManager`
- If no idle carrier exists, training completion is deferred (progress stays at 1.0)

**Events emitted** (via EventBus):
- `barracks:trainingStarted` — `{ buildingId, recipe: TrainingRecipe }`
- `barracks:trainingCompleted` — `{ buildingId, unitType: UnitType, level: number, soldierId: number }`
- `barracks:trainingDeferred` — `{ buildingId, reason: 'no_carrier' | 'no_spawn_tile' }`

### Subsystem 5: Integration & Wiring
...
**Owns**: Lifecycle wiring, construction spawn config update, production chain cleanup.

**Depends on**: Subsystem 3 (recipe types), Subsystem 4 (manager class).
...
3. **Training lifecycle tests** (using TestGame helper):
   - Build barracks → deposit SWORD ×1 → tick → training starts → tick to completion → swordsman L1 spawned + carrier consumed
   - Deposit SWORD + GOLDBAR → trains Swordsman L2 (if selected)
   - Deposit BOW → trains Bowman L1
   - Deposit SWORD + ARMOR → trains SquadLeader

4. **Carrier consumption tests**:
   - Training completes with idle carrier available → carrier removed, soldier spawned
   - Training completes with no idle carrier → training deferred (progress=1.0), retries next tick
   - Carrier becomes available on later tick → training completes

5. **Recipe selection integration**:
...
| `tests/unit/economy/barracks-training.spec.ts` | 6 | Barracks training tests |
...
| `src/game/economy/building-production.ts` | 5 | Remove Barrack from BUILDING_PRODUCTIONS, update getBuildingTypesRequestingMaterial |
| `src/game/features/building-construction/spawn-units.ts` | 5 | Remove Barrack from BUILDING_SPAWN_ON_COMPLETE |
| `src/game/game-services.ts` | 5 | Instantiate and wire BarracksTrainingManager, update PCM init call sites |
| `src/game/game.ts` | 5 | Register barracksTrainingManager.tick() in game loop |
| `tests/unit/economy/production-system.spec.ts` | 6 | Update existing PCM tests for recipe-index API |
...
3. **Carrier consumption source**: Should the consumed carrier come from the nearest tavern specifically, or any idle carrier owned by the player? Design prefers nearest to the barracks for visual coherence (the carrier visually "walks in and transforms"), but functionally any idle carrier works.

4. **CHANGE_TYPE_AT_BARRACKS choreography node**: The existing stub executor in `control-executors.ts` is not used by this design. The barracks training system operates as a tick-based manager, not through the choreography system. The stub can remain as-is or be removed. If the original S4 choreography (carrier walks into barracks, emerges as soldier) is desired later, it would be a separate enhancement layered on top.
...
- **Carrier-walks-in animation**: The original S4 shows the carrier walking into the barracks and emerging as a soldier. This design uses instant carrier removal + soldier spawn. A choreography-based approach could be added later.
- **CHANGE_TYPE_AT_BARRACKS executor**: The existing choreography stub is not implemented or removed — it remains as-is for potential future use.
