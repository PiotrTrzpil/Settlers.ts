...
describe('TerritoryMap', () => {
    ...
    it('should start with all tiles unowned', () => {
        ...
        expect(territory.getOwner(10, 10)).to.equal(NO_OWNER);
        expect(territory.getOwner(32, 32)).to.equal(NO_OWNER);
    });
    ...
    it('should claim territory around a building', () => {
        ...
        expect(territory.getOwner(20, 20)).to.equal(0);
        // Tile at (25, 20) is 5 tiles away, within radius 8
        expect(territory.getOwner(25, 20)).to.equal(0);
        // Tile at (30, 20) is 10 tiles away, outside radius 8
        expect(territory.getOwner(30, 20)).to.equal(NO_OWNER);
    });
    ...
    it('should use circular territory shape', () => {
        ...
        expect(territory.getOwner(26, 26)).to.equal(NO_OWNER);
        // Diagonal at (25, 25) is ~7.1 away, inside radius 8
        expect(territory.getOwner(25, 25)).to.equal(0);
    });
    ...
    it('should use different radii for different building types', () => {
        ...
        expect(territory.getOwner(13, 10)).to.equal(0); // 3 tiles away
        expect(territory.getOwner(15, 10)).to.equal(NO_OWNER); // 5 tiles away
    });
    ...
    it('should handle multiple players', () => {
        ...
        expect(territory.getOwner(10, 10)).to.equal(0);
        expect(territory.getOwner(40, 40)).to.equal(1);
        expect(territory.isOwnedBy(10, 10, 0)).to.equal(true);
        expect(territory.isOwnedBy(40, 40, 1)).to.equal(true);
    });
    ...
    it('should resolve overlap by closest building', () => {
        ...
        expect(territory.getOwner(25, 20)).to.equal(0); // 5 from P0, 7 from P1
        expect(territory.getOwner(27, 20)).to.equal(1); // 7 from P0, 5 from P1
    });
    ...
    it('should clear territory on rebuild with no buildings', () => {
        ...
        territory.rebuild([]);
        expect(territory.getOwner(20, 20)).to.equal(NO_OWNER);
    });
